...
- **`face`: one region per detected face, largest first.** The underlying model (`data/models/face.npz`) is still trained on a single face per image, not an ensemble -- but that same probe now runs over every face the detector finds (down to the same minimum size as before) rather than only the largest, so a photo with multiple people can produce multiple `face` regions, each with its own real `score`/`verdict`. `signals.face` and `face`'s contribution to the headline `confidence` are the MAX across those regions' scores: one AI-looking face is enough to flag the image, same logic as `max(general, tampered, face)` itself. A photo with no face still produces zero `face` regions and `signals.face: null`, unchanged.
